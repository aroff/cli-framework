...
| `self-install` | off | `self install` / `update` / `rollback` / `uninstall` / `status` for `EndUser` apps (ADR 0080): install receipt, reversible PATH edits (shared `env` file on Unix, `HKCU\Environment` on Windows), `SHA256SUMS` and optional minisign verification, `self_update.*` policy keys, opt-in update notice, Windows Apps & Features entry, four `install.*` doctor checks, `SI001`–`SI011` codes. Enable with `AppBuilder::with_self_install(SelfInstallOptions::github(..))`. Implies `doctor`, `config`. See `self-install-and-distribution.md` |
...
| `telemetry` | off | Built-in OpenTelemetry (ADRs 0068, 0076–0079). Auto `cli.command` spans + `cli.command.invocations` / `cli.command.duration_ms` at every dispatch seam, a person-controlled level (`off` < `usage` < `diagnostic` < `debug`) gating a probe catalogue, redacting export boundary, kill switches (`<APP>_TELEMETRY_DISABLED`, `OTEL_SDK_DISABLED`, `DO_NOT_TRACK`), first-run notice and `telemetry` command group on `EndUser` installs, six `doctor` checks, W3C context propagation, OTLP auth headers. Configure with `AppBuilder::with_deployment` + `with_telemetry_defaults`; `with_telemetry(TelemetryConfig)` is deprecated (v0.6.0, removed v0.8.0). OTLP `http/protobuf` only. Implies `observability`, `config`, `doctor`. Hold `install_default_logging()`'s guard instead of installing your own subscriber |
