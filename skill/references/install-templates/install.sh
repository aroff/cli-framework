#!/bin/sh
# __APP__ installer script for Linux and macOS.
#
#   curl -fsSL https://github.com/__REPO__/releases/latest/download/install.sh | sh
#   curl -fsSL https://github.com/__REPO__/releases/download/__TAG_PREFIX__1.4.2/install.sh | sh
#
# Environment (all optional):
#   __APP_ENV___VERSION             stable (default) | latest | 1.4.2
#   __APP_ENV___INSTALL_DIR         bin dir (default: $XDG_BIN_HOME or ~/.local/bin)
#   __APP_ENV___NO_MODIFY_PATH=1    leave shell rc files untouched
#   __APP_ENV___UNMANAGED=1         no PATH edit, no receipt (images, CI)
#   __APP_ENV___INSTALLER_BASE_URL  https mirror serving the same asset names
#                                   (plain http only for 127.0.0.1, localhost, [::1])
#   __APP_ENV___GITHUB_TOKEN / GITHUB_TOKEN   private GitHub releases (header
#                                   only; never sent to a mirror)
#   __APP_ENV___INSTALL_ALLOW_SUDO=1          allow running as root
#
# This script only downloads and verifies. Placement, PATH and the install
# receipt are done by `__APP__ self install`, so this file rarely changes.
...
main() {
  ...
  SELF_CMD="__SELF_CMD__"
  VERSION="${__APP_ENV___VERSION:-stable}"
  TOKEN="${__APP_ENV___GITHUB_TOKEN:-${GITHUB_TOKEN:-}}"
  MIRROR="${__APP_ENV___INSTALLER_BASE_URL:-}"
  PROTO='=https'
  if [ -n "$MIRROR" ]; then
    ...
    case "$MIRROR" in
      ...
      http://127.0.0.1[:/]*|http://127.0.0.1|http://localhost[:/]*|http://localhost|http://\[::1\][:/]*|http://\[::1\])
        PROTO='=http,https' ;;
      *) die "__APP_ENV___INSTALLER_BASE_URL must be https (plain http is allowed only for a loopback address)" ;;
    esac
  fi

  if [ "$(id -u)" = "0" ] && [ -z "${__APP_ENV___INSTALL_ALLOW_SUDO:-}" ]; then
    die "refusing to install as root: it would land in root's home. Set __APP_ENV___INSTALL_ALLOW_SUDO=1 to override."
  fi
  ...
  bin_dir="${__APP_ENV___INSTALL_DIR:-$default_bin}"
  ...
  [ -n "${__APP_ENV___NO_MODIFY_PATH:-}" ] && set -- "$@" --no-modify-path
  [ -n "${__APP_ENV___UNMANAGED:-}" ] && set -- "$@" --unmanaged
