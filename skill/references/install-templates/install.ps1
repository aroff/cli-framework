# __APP__ installer script for Windows.
#
#   irm https://github.com/__REPO__/releases/latest/download/install.ps1 | iex
#   irm https://github.com/__REPO__/releases/download/__TAG_PREFIX__1.4.2/install.ps1 | iex
#
# Environment (all optional):
#   __APP_ENV___VERSION             stable (default) | latest | 1.4.2
#   __APP_ENV___INSTALL_DIR         bin dir (default: $HOME\.local\bin)
#   __APP_ENV___NO_MODIFY_PATH=1    leave the user PATH untouched
...
function Install-__APP__ {
  ...
  $App = '__APP__'
  $Repo = '__REPO__'
  $TagPrefix = '__TAG_PREFIX__'
  # How the self group is reached: 'self', or 'cli self' when the app puts
  # its built-in commands under a namespace.
  $SelfCmd = @('__SELF_CMD__' -split ' ')
  $Version = if ($env:__APP_ENV___VERSION) { $env:__APP_ENV___VERSION } else { 'stable' }
  $Token = if ($env:__APP_ENV___GITHUB_TOKEN) { $env:__APP_ENV___GITHUB_TOKEN } elseif ($env:GITHUB_TOKEN) { $env:GITHUB_TOKEN } else { $null }
  $Mirror = $env:__APP_ENV___INSTALLER_BASE_URL
  if ($Mirror) {
    # A mirror never receives the GitHub token.
    $Token = $null
    $uri = [Uri]$Mirror
    if ($uri.Scheme -ne 'https' -and -not ($uri.Scheme -eq 'http' -and $uri.IsLoopback)) {
      throw '__APP_ENV___INSTALLER_BASE_URL must be https (plain http is allowed only for a loopback address)'
    }
  }
  $Headers = @{}
  ...
  $binDir = if ($env:__APP_ENV___INSTALL_DIR) { $env:__APP_ENV___INSTALL_DIR }
  ...
  try {
    ...
    if ($env:__APP_ENV___NO_MODIFY_PATH) { $cliArgs += '--no-modify-path' }
    if ($env:__APP_ENV___UNMANAGED) { $cliArgs += '--unmanaged' }
