...
| `self_install` (feature `self-install`) | ADR 0080 phase 1. `InstallEnv` snapshots the process (OS, home, XDG/known-folder roots, PATH, env vars, TTY) so every operation is testable without touching the real machine. `options` (`SelfInstallOptions`), `layout` (bin dir, receipt path, asset names), `receipt` (`InstallReceipt`, versioned JSON), `method` (script / unmanaged / package-manager inference), `place` (same-directory temp + rename, Windows `.exe.old` swap, `startup_cleanup`), `shell_path` (shared `env`/`env.fish`, rc lines in existing files only), `windows_path` (`HKCU\Environment` `Path` + `WM_SETTINGCHANGE`), `ops` (`install`/`uninstall`/`status`), `commands` (the `self` group, registered by `AppBuilder::with_self_install` for `EndUser` only) and `doctor` (`install.*` checks). Installer script templates live in `skill/references/install-templates/`; `scripts/ci/self-install-e2e.{sh,ps1}` test them against a loopback release. |
